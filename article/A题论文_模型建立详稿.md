...
为避免仅以“程序正常运行”作为模型可信性的依据，本文从离散收敛、终止判据、对照试验和机理一致性四个层面对问题四进行专项检验。首先，在保持其余参数不变的条件下，将初始名义径向步长由 $0.0125\ \mathrm{cm}$ 加密至 $0.00625\ \mathrm{cm}$，所得干燥时间分别为 $51.118333\ \mathrm h$ 和 $51.117222\ \mathrm h$，仅相差 $4\ \mathrm s$，相对偏差为 $0.0022\%$。在粗网格上进一步将三阶段时间步由 $2/60/2\ \mathrm s$ 减半为 $1/30/1\ \mathrm s$，干燥时间改变 $72\ \mathrm s$，相对偏差为 $0.0391\%$。粗细网格公共输出点的含水率最大差和平均差分别为 $1.857\times10^{-4}\ \mathrm{kg/kg}$ 和 $5.288\times10^{-6}\ \mathrm{kg/kg}$。这些误差均远小于参数扰动引起的小时量级变化，说明基准结果已基本摆脱网格和时间步选择的影响。

终止判据采用未舍入场量进行夹逼。达标前一时刻全域最大含水率为 $0.1500003574\ \mathrm{kg/kg}$，达标时为 $0.1499998440\ \mathrm{kg/kg}$，且控制位置始终位于圆柱中心。因此，$t_d=51.1172\ \mathrm h$ 确为模型首次满足

$$
\max_{0\le r\le R(t)}C(r,t)<0.15\ \mathrm{kg/kg}
$$

的时刻，而不是由结果保留位数或输出间隔造成的提前判定。

为检验引入移动边界的必要性，本文在附录 4 物性、环境边界、初始条件、传热传质系数和数值格式完全相同的条件下，仅将实测半径 $R(t)$ 替换为固定半径 $R_0=2\ \mathrm{cm}$。固定半径与收缩半径模型的干燥时间分别为 $129.8869\ \mathrm h$ 和 $51.1172\ \mathrm h$；收缩模型所需时间仅为固定半径模型的 $39.36\%$。当收缩模型达到阈值时，固定半径模型的中心含水率仍为 $0.3049\ \mathrm{kg/kg}$。这一控制变量对照表明，尺寸收缩对内部传质路径的影响属于决定性量级，不能用固定区域模型替代。

进一步定义移动边界下的等效扩散时钟

$$
\Theta_{\mathrm{mov}}(t)=\int_0^t\frac{D_{\mathrm{eff}}(s)}{R^2(s)}\,\mathrm ds,
\qquad
\Theta_{\mathrm{fix}}(t)=\int_0^t\frac{D_{\mathrm{eff}}(s)}{R_0^2}\,\mathrm ds,
$$

其中 $D_{\mathrm{eff}}$ 为截面加权调和平均扩散系数。干燥结束时，二者分别为 $0.6924$ 和 $0.3086$，几何增益 $\Theta_{\mathrm{mov}}/\Theta_{\mathrm{fix}}=2.2437$，从独立后处理角度证明半径缩小显著提高了累计扩散效率。与此同时，以瞬时半径为特征长度得到的有效质量 Biot 数满足 $Bi_{m,\mathrm{eff}}=5.39\sim126.71>1$，说明主要阻力位于药材内部；这与“中心最后达标”的空间特征相一致。

最后进行单因素局部灵敏度检验。对传热系数 $h$、传质系数 $h_m$、水分扩散系数 $D$ 和收缩幅度 $s_R$ 分别施加 $\pm20\%$ 扰动，对 $4\ \mathrm h$ 后恒温段温度 $T_\infty$ 施加 $\pm2\ ^\circ\mathrm C$ 扰动。定义归一化灵敏度

$$
S_p=\frac{\Delta t_d/t_d}{\Delta p/p}.
$$

各参数灵敏度绝对值由大到小依次为：恒温段温度 $1.5690$、收缩幅度 $1.1287$、水分扩散系数 $0.9162$、表面对流传质系数 $0.0527$、表面对流换热系数 $0.0015$。其中，温度降低和升高 $2\ ^\circ\mathrm C$ 时，干燥时间分别为 $54.4672\ \mathrm h$ 和 $48.0506\ \mathrm h$；收缩幅度降低和提高 $20\%$ 时，分别为 $63.5072\ \mathrm h$ 和 $40.4289\ \mathrm h$；扩散系数降低和提高 $20\%$ 时，分别为 $62.3783\ \mathrm h$ 和 $43.6450\ \mathrm h$。所有灵敏度均为负，表明在当前参数邻域内增大这些参数都会缩短干燥时间。恒温段温度、收缩规律和内部扩散系数的影响远大于数值离散误差，而 $h$ 与 $h_m$ 的影响较小，进一步支持“后期干燥受内部扩散控制”的机理判断。

综合上述检验，第四问的 $51.1172\ \mathrm h$ 结果在数值上稳定，阈值定位明确，移动边界效应具有独立对照支撑，灵敏度排序也与 Biot 数和控制位置相互印证。需要说明的是，前述 $2.20\times10^{-13}$ 残差只证明等效水分离散方程闭合；若将附件 2 的收缩半径、附录 4 的湿基密度、长度不变和均匀径向收缩同时用于反算干物质质量，终止时干物质代理量相对初值偏移约 $-10.86\%$。该偏移反映经验收缩关系与密度关系并非严格质量相容，不应解释为数值误差。因此，本模型适用于题设工况下的干燥时间与场分布预测；推广到其他尺寸或收缩方式时，应重新标定收缩—密度—含水率关系。
...
问题四相对问题三同时改变了几何描述和经验物性体系，因此上述 $11.11\%$ 是“收缩移动边界与附录 4 参数”的综合影响，不能全部归因于尺寸收缩。为分离几何效应，本文又在相同附录 4 物性下设置固定半径对照，所得干燥时间为 $129.8869\ \mathrm h$；与收缩模型的 $51.1172\ \mathrm h$ 相比，表明半径减小通过缩短内部扩散路径、改变表面积体积比而显著加快干燥。
...
9. 问题四相对问题三的时间差是几何与物性同时变化的综合结果；纯粹的收缩贡献应引用本文在同一套附录 4 物性下设置的固定半径对照结果。
