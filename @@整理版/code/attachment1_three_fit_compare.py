...
def plot_comparison(
    ...
    temperature: dict[str, np.ndarray], moisture: dict[str, np.ndarray],
    temp_loocv: dict[str, np.ndarray], moisture_loocv: dict[str, np.ndarray],
) -> None:
    configure_fonts()
    fig, axes = plt.subplots(3, 2, figsize=(7.20, 7.25), constrained_layout=True,
    ...
    for col, (observed, prediction, unit, title) in enumerate([
        ...
        axes[0, col].scatter(obs_minute, observed, s=9, color="#222222", zorder=4,
                             label="附件1观测点")
        axes[0, col].set(title=f"({'ab'[col]}) {title}拟合", ylabel=f"数值 / ({unit})", xlim=(0, 30))
        ...
        axes[1, col].set(title=f"({'cd'[col]}) 相对分段线性的差值", ylabel=f"差值 / ({unit})", xlim=(0, 30))
        axes[1, col].legend(loc="best")

    for col, (observed, cv, unit, title) in enumerate([
        (temperature_obs, temp_loocv, "°C", "温度"),
        (moisture_obs, moisture_loocv, "kg/kg", "水分浓度"),
    ]):
        interior = slice(1, -1)
        for method in ["linear", "pchip", "stretched"]:
            error = np.abs(cv[method][interior] - observed[interior])
            axes[2, col].plot(obs_minute[interior], error, color=colors[method],
                              ls=styles[method], lw=1.05, marker="o", ms=2.4,
                              label=labels[method])
        axes[2, col].set(title=f"({'ef'[col]}) {title}留一预测误差", xlabel="时间 / min",
                         ylabel=f"绝对误差 / ({unit})", xlim=(0, 30))
    ...
    fig.suptitle("附件1三种边界曲线构造方法对比", fontsize=10.0, fontweight="bold")
...
def main() -> None:
    ...
    plot_comparison(picture_dir / "attachment1_three_method_comparison.png",
                    times_s, temperature_obs, moisture_obs, query_times_s,
                    temperature, moisture, temp_loocv, moisture_loocv)
