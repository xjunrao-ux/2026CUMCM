...
保持预处理后的分段线性边界和其余参数不变，每次只改变一个参数。
响应仍采用30 min温升与失水量的相对变化，扫描区间、阈值和绘图方法
均保持原a1_parameter_sensitivity.py不变。
"""
...
def configure_plot() -> None:
    ...
    fonts = [
        name
        for name in ["Microsoft YaHei", "SimHei", "Arial", "DejaVu Sans"]
...
def reset_parameters() -> None:
    model.DENSITY_KG_M3 = BASELINE["rho"]
    model.SPECIFIC_HEAT_J_KG_K = BASELINE["cp"]
    model.THERMAL_CONDUCTIVITY_W_M_K = BASELINE["k"]
    model.CONVECTION_COEFFICIENT_W_M2_K = BASELINE["h"]
...
def set_parameter(key: str, value: float) -> None:
    if key == "rho":
        model.DENSITY_KG_M3 = value
    elif key == "cp":
        model.SPECIFIC_HEAT_J_KG_K = value
    elif key == "k":
        model.THERMAL_CONDUCTIVITY_W_M_K = value
    elif key == "h":
        model.CONVECTION_COEFFICIENT_W_M2_K = value
    elif key == "hm":
...
def run_temperature(environment: model.EnvironmentData) -> RunMetrics:
    result = model.simulate_temperature(
        environment.times_s,
...
def response_changes(
    metrics: RunMetrics, baseline: RunMetrics, domain: str
) -> dict[str, float]:
    if domain == "temperature":
        ...
        for point in ["center", "surface", "average"]:
            ...
            values[f"{point}_response_change_pct"] = (
                ...
                * (values[f"{point}_moisture_loss_kg_kg"] / denominator - 1.0)
            )
    return values
...
def max_relevant_change(row: dict[str, float], domain: str) -> float:
    points = (
        ["center", "surface", "average"]
        if domain == "temperature"
        else ["surface", "average"]
    )
    return max(abs(row[f"{point}_response_change_pct"]) for point in points)


def crossing_factor(
    rows: list[dict[str, float]], target: float, side: str
) -> float | None:
    ordered = sorted(rows, key=lambda row: row["factor"])
    base_index = next(
        i for i, row in enumerate(ordered) if math.isclose(row["factor"], 1.0)
    )
    indices = (
        range(base_index, -1, -1)
        if side == "lower"
        else range(base_index, len(ordered))
    )
    previous = ordered[base_index]
    for index in indices:
        current = ordered[index]
        y0 = previous["max_abs_response_change_pct"]
        y1 = current["max_abs_response_change_pct"]
        if y1 >= target and not math.isclose(current["factor"], 1.0):
            ...
            return float(
                previous["factor"]
                + fraction * (current["factor"] - previous["factor"])
            )
        previous = current
    return None
...
def interpolate_change(rows: list[dict[str, float]], factor: float) -> float:
    factors = np.asarray([row["factor"] for row in rows])
    changes = np.asarray([row["max_abs_response_change_pct"] for row in rows])
...
def plot_response_curves(path: Path, grouped: dict[str, list[dict]]) -> None:
    ...
    for axis, domain, point, title in panels:
        for key, rows in grouped.items():
            if PARAMETERS[key][2] != domain:
                continue
            x = np.asarray([row["factor"] for row in rows])
            y = np.asarray([row[f"{point}_response_change_pct"] for row in rows])
            axis.plot(
                x,
                y,
                color=palette[key],
        ...
        axis.axhspan(-5, 5, color="#D9EAD3", alpha=0.35, zorder=0)
        axis.axhline(10, color="#666666", ls="--", lw=0.8)
        axis.axhline(-10, color="#666666", ls="--", lw=0.8)
        axis.axvline(1.0, color="#333333", lw=0.7)
        axis.set(
            title=title,
            xlabel="参数倍率（相对基准）",
            ylabel="响应相对变化 / %",
        ...
        axis.grid(True, color="#E2E2E2", lw=0.45)
        axis.legend(frameon=False, ncol=2, loc="best")
    fig.suptitle(
        "A题问题1参数灵敏度：30 min温升与失水响应",
        fontsize=10.0,
        fontweight="bold",
    )
...
def plot_tornado(path: Path, grouped: dict[str, list[dict]]) -> None:
    ...
    lower_array = np.asarray(lower)[order]
    upper_array = np.asarray(upper)[order]
    y = np.arange(len(labels))
    fig, axis = plt.subplots(figsize=(7.20, 3.65), constrained_layout=True)
    axis.barh(y - 0.18, lower_array, height=0.34, color="#56B4E9", label="参数降至0.8倍")
    axis.barh(y + 0.18, upper_array, height=0.34, color="#D55E00", label="参数增至1.2倍")
    axis.axvline(5, color="#888888", ls=":", lw=0.9)
    axis.axvline(10, color="#555555", ls="--", lw=0.9)
    axis.set(
        yticks=y,
        yticklabels=labels,
        xlabel="最大响应变化 / %",
    ...
    axis.grid(axis="x", color="#E2E2E2", lw=0.45)
...
def plot_threshold_table(path: Path, thresholds: list[dict]) -> None:
    ...
    for row in thresholds:
        unit = row["unit"]
        baseline = (
            f'{row["baseline_value"]:.3g} {unit}'
            if unit != "—"
            else f'{row["baseline_value"]:.3g}'
        )
        cells.append(
            [
                row["parameter_name"],
    ...
    fig, axis = plt.subplots(figsize=(7.20, 3.25), constrained_layout=True)
    axis.axis("off")
    table = axis.table(
        cellText=cells,
        colLabels=headers,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--result-dir",
        type=Path,
        default=project_root / "results" / "A_problem1_modular" / "sensitivity",
    )
    parser.add_argument(
        "--figure-dir",
        type=Path,
        default=project_root / "picture" / "A_problem1_modular_sensitivity",
    )
...
def main() -> None:
    ...
    for key, (name, unit, domain) in PARAMETERS.items():
        ...
        for factor in FACTOR_GRID:
            ...
            metrics = (
                run_temperature(environment)
                if domain == "temperature"
                else run_moisture(environment)
            )
        ...
        thresholds.append(
            {
                ...
                "lower_value_10pct": None
                if lower10 is None
                else BASELINE[key] * lower10,
                "upper_value_10pct": None
                if upper10 is None
                else BASELINE[key] * upper10,
    ...
    rho_cp_difference = max(
        abs(a["max_abs_response_change_pct"] - b["max_abs_response_change_pct"])
        for a, b in zip(grouped["rho"], grouped["cp"])
    )
    report = {
        "method": "one-at-a-time factor sweep",
        "factor_grid": FACTOR_GRID.tolist(),
        "large_difference_definition": (
            "at least 10% change in temperature rise or moisture loss"
        ),
        "warning_definition": (
            "at least 5% change in temperature rise or moisture loss"
        ),
