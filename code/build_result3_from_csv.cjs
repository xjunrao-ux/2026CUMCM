...
async function main() {
  ...
  if (!templatePath || !csvPath || !outputPath || !previewDir) {
    throw new Error(
      "Usage: node build_result3_from_csv.cjs TEMPLATE CSV OUTPUT PREVIEW_DIR",
    );
  }
  ...
  await fs.writeFile(
    path.join(previewDir, "result3_inspection.ndjson"),
  ...
  for (const [label, range] of [
    ...
    await fs.writeFile(
      path.join(previewDir, `result3_${label}.png`),
      new Uint8Array(await preview.arrayBuffer()),
    );
  }

  console.log(`Verified result3.xlsx: ${rows.length} data rows.`);
}
