...
const templatePath = `${root}/比赛题目/CUMCM2026Problems/A题/附件/附件3/result1.xlsx`;
const resultDir = `${root}/results/A_problem1_coupled`;
const outputPath = `${resultDir}/result1.xlsx`;

function parseNumericCsv(text) {
  const lines = text.replace(/^\uFEFF/, "").trim().split(/\r?\n/);
  const rows = lines.slice(1).map((line) => line.split(",").map(Number));
  if (rows.length !== 1801 || rows.some((row) => row.length !== 22)) {
    throw new Error("Expected 1801 data rows and 22 columns in each result CSV.");
  }
  return rows;
}
...
for (const [sheetName, rows] of [
  ...
  sheet.getRange("A1:V1802").values = matrix;
  sheet.getRange("A2:A1802").format.numberFormat = "0";
  sheet.getRange("B2:V1802").format.numberFormat = "0.0000";
  ...
  sheet.getRange("A2:V1802").format.horizontalAlignment = "right";
...
for (const sheetName of ["温度", "水分浓度"]) {
  for (const [label, range] of [["top", "A1:V8"], ["bottom", "A1795:V1802"]]) {
